/*+ Rows(it1 pi1 n an ci #577) Rows(rt #5) Rows(it1 pi1 n an #43) Rows(ci #13) Rows(it1 #1) Rows(pi1 n an #43) NestLoop(an ci it1 n pi1 rt) NestLoop(an ci it1 n pi1) NestLoop(an it1 n pi1) NestLoop(an n pi1) NestLoop(n pi1)  Leading((((it1 ((pi1 n) an)) ci) rt)) SeqScan(rt) IndexScan(ci) SeqScan(it1) IndexOnlyScan(an) BitmapScan(pi1) IndexScan(n)   */ explain (format json) SELECT COUNT(*)
