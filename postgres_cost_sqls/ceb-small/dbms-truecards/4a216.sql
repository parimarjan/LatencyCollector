/*+ Rows(it1 pi1 an n ci #375478) Rows(rt #8) Rows(it1 pi1 an n #713) Rows(ci #527) Rows(it1 #1) Rows(pi1 an n #713) HashJoin(an ci it1 n pi1 rt) NestLoop(an ci it1 n pi1) NestLoop(an it1 n pi1) NestLoop(an n pi1) HashJoin(an pi1)  Leading((((it1 ((pi1 an) n)) ci) rt)) SeqScan(rt) IndexScan(ci) SeqScan(it1) IndexScan(n) BitmapScan(pi1) SeqScan(an)   */ explain (format json) SELECT COUNT(*)
