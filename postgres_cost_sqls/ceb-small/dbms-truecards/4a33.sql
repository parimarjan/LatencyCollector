/*+ Rows(it1 n an pi1 ci #3466) Rows(rt #1) Rows(it1 n an pi1 #43) Rows(ci #81) Rows(it1 #1) Rows(n an pi1 #43) NestLoop(an ci it1 n pi1 rt) NestLoop(an ci it1 n pi1) NestLoop(an it1 n pi1) NestLoop(an n pi1) NestLoop(an n)  Leading((((it1 ((n an) pi1)) ci) rt)) SeqScan(rt) IndexScan(ci) SeqScan(it1) IndexScan(pi1) SeqScan(n) IndexOnlyScan(an)   */ explain (format json) SELECT COUNT(*)
