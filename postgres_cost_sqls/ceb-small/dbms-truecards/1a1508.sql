/*+ Rows(it2 #1) Rows(it1 mi1 kt t mi2 ci rt n #2365) NestLoop(ci it1 it2 kt mi1 mi2 n rt t) NestLoop(ci it1 kt mi1 mi2 n rt t) NestLoop(ci kt mi1 mi2 n rt t) HashJoin(ci kt mi1 mi2 rt t) NestLoop(ci kt mi1 mi2 t) NestLoop(kt mi1 mi2 t) HashJoin(kt mi1 t) NestLoop(kt t)  Leading((it2 (it1 (((((mi1 (kt t)) mi2) ci) rt) n)))) SeqScan(it2) SeqScan(it1) IndexScan(n) SeqScan(rt) IndexScan(ci) IndexScan(mi2) BitmapScan(mi1) SeqScan(kt) BitmapScan(t)   */ explain (format json) SELECT COUNT(*) FROM info_type as it2 CROSS JOIN (info_type as it1 CROSS JOIN (((((movie_info as mi1 CROSS JOIN (kind_type as kt CROSS JOIN title as t)) CROSS JOIN movie_info as mi2) CROSS JOIN cast_info as ci) CROSS JOIN role_type as rt) CROSS JOIN name as n)) WHERE
t.id = ci.movie_id
