/*+ Rows(rt t kt mi2 ci mi1 it1 it2 #12) Rows(n #0) Rows(rt t kt mi2 ci mi1 it1 #12) Rows(it2 #1) Rows(rt t kt mi2 ci mi1 #12) Rows(it1 #1) Rows(rt t kt mi2 ci #12) Rows(mi1 #1) Rows(rt #1) Rows(t kt mi2 ci #608) NestLoop(ci it1 it2 kt mi1 mi2 n rt t) NestLoop(ci it1 it2 kt mi1 mi2 rt t) NestLoop(ci it1 kt mi1 mi2 rt t) NestLoop(ci kt mi1 mi2 rt t) NestLoop(ci kt mi2 rt t) NestLoop(ci kt mi2 t) NestLoop(kt mi2 t) NestLoop(kt t)  Leading((((((rt (((t kt) mi2) ci)) mi1) it1) it2) n)) IndexScan(n) SeqScan(it2) SeqScan(it1) IndexScan(mi1) SeqScan(rt) IndexScan(ci) IndexScan(mi2) SeqScan(t) SeqScan(kt)   */ explain (format json) SELECT COUNT(*) FROM ((((role_type as rt CROSS JOIN (((title as t CROSS JOIN kind_type as kt) CROSS JOIN movie_info as mi2) CROSS JOIN cast_info as ci)) CROSS JOIN movie_info as mi1) CROSS JOIN info_type as it1) CROSS JOIN info_type as it2) CROSS JOIN name as n WHERE
t.id = ci.movie_id
