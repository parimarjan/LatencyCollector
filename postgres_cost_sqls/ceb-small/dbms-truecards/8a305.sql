/*+ Rows(rt ct cn mc t kt mk ci mi1 it1 n #18424) Rows(k #1) Rows(rt ct cn mc t kt mk ci mi1 it1 #644883) Rows(n #0) Rows(rt ct cn mc t kt mk ci mi1 #644883) Rows(it1 #1) Rows(rt ct cn mc t kt mk ci #675076) Rows(mi1 #1) Rows(rt #1) Rows(ct cn mc t kt mk ci #1493263) NestLoop(ci cn ct it1 k kt mc mi1 mk n rt t) NestLoop(ci cn ct it1 kt mc mi1 mk n rt t) NestLoop(ci cn ct it1 kt mc mi1 mk rt t) NestLoop(ci cn ct kt mc mi1 mk rt t) NestLoop(ci cn ct kt mc mk rt t) NestLoop(ci cn ct kt mc mk t) NestLoop(cn ct kt mc mk t) NestLoop(cn ct kt mc t) NestLoop(cn ct mc t) NestLoop(cn ct mc) NestLoop(cn mc)  Leading((((((rt (((((ct (cn mc)) t) kt) mk) ci)) mi1) it1) n) k)) IndexOnlyScan(k) IndexScan(n) SeqScan(it1) IndexScan(mi1) SeqScan(rt) IndexScan(ci) IndexScan(mk) SeqScan(kt) IndexScan(t) SeqScan(ct) SeqScan(cn) IndexScan(mc)   */ explain (format json) SELECT COUNT(*) FROM ((((role_type as rt CROSS JOIN (((((company_type as ct CROSS JOIN (company_name as cn CROSS JOIN movie_companies as mc)) CROSS JOIN title as t) CROSS JOIN kind_type as kt) CROSS JOIN movie_keyword as mk) CROSS JOIN cast_info as ci)) CROSS JOIN movie_info as mi1) CROSS JOIN info_type as it1) CROSS JOIN name as n) CROSS JOIN keyword as k WHERE
t.id = ci.movie_id
