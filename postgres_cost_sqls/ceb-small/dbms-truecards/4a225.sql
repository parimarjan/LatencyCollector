/*+ Rows(it1 an n pi1 ci #39815) Rows(rt #6) Rows(it1 an n pi1 #555) Rows(ci #72) Rows(it1 #1) Rows(an n pi1 #555) HashJoin(an ci it1 n pi1 rt) NestLoop(an ci it1 n pi1) NestLoop(an it1 n pi1) NestLoop(an n pi1) HashJoin(an n)  Leading((((it1 ((an n) pi1)) ci) rt)) SeqScan(rt) IndexScan(ci) SeqScan(it1) IndexScan(pi1) SeqScan(an) SeqScan(n)   */ explain (format json) SELECT COUNT(*)
